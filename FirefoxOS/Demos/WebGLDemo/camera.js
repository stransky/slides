...
function handleCameraControl() {
  ...
  // There's an ongoing touch event
  if(ongoingTouchId != -1) {
    graph.camera.rotation -= Math.PI*0.002*ongoingTouchDx;
    graph.camera.elevation += Math.PI*0.002*ongoingTouchDy;
    graph.camera.changed = 1;  
  }
  
  if(!graph.camera.changed) {
    graph.camera.rotation -= Math.PI*0.004;
    graph.camera.changed = 1;
  }
}
